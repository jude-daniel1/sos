...
class UserConfirmCode extends StatefulWidget {
  String? email, code;
  UserConfirmCode(this.email, this.code, {Key? key}) : super(key: key);

  @override
  _UserConfirmCodeState createState() => _UserConfirmCodeState();
}

class _UserConfirmCodeState extends State<UserConfirmCode> {
  ...
  resendCode() async {
    ...
    try {
      Response? response =
          await sendVerificationCode("send_code", widget.email!);
      ...
      if (widget.code == supliedCode) {
        ...
        Timer.periodic(Duration(seconds: 3), (timer) {
          ...
          Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                  builder: (_) => UserRegister(widget.email ?? "")));
        });
      } else {
        _codeTextEditingController.clear();
        var alertStyle = AlertStyle(
